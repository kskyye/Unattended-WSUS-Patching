...
$updateClassifications = $wsus.GetUpdateClassifications() | Where {
  $_.Title -Match "Critical Updates|Security Updates|Service Packs|Update Rollups|Security"
}
...
$report += "The $($Updates.count) Windows updates listed in the attached report have been approved and a deadline has been set for $($TargetGroup.name)."
$email.body = $report
$smtp.Send($email)
$attachment.Dispose()
